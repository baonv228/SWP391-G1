...
/**
 * Truy van bang PasswordResetToken (SQL Server) cho chuc nang Password Reset.
 */
public class PasswordResetTokenDao extends DBContext {

    // Tao token moi
    public boolean create(int userId, String token, Timestamp expiresAt) {
        String sql = "INSERT INTO dbo.PasswordResetToken(UserID, Token, ExpiresAt, Used) VALUES (?, ?, ?, 0)";
    ...
    public boolean markUsed(int tokenId) {
        String sql = "UPDATE dbo.PasswordResetToken SET Used = 1 WHERE TokenID = ?";
        try (Connection con = getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, tokenId);
            return ps.executeUpdate() > 0;
        } catch (Exception e) {

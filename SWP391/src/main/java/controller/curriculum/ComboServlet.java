...
@WebServlet(name = "CurriculumComboServlet", urlPatterns = {"/curriculum/combo"})
public class ComboServlet extends HttpServlet {
    ...
    private void showList(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Combo> comboList = comboDAO.getComboByCurriculum(curriculumId);
            request.setAttribute("curriculum", curriculum);
            request.setAttribute("comboList", comboList);
            request.setAttribute("curriculumId", curriculumId);

            // Forward to standard view folder, check if we need to support both by checking URL or keeping standard
            request.getRequestDispatcher("/view/curriculum/combo-management.jsp").forward(request, response);
        } catch (SQLException e) {
            getServletContext().log("Database error in ComboServlet.showList", e);
            request.setAttribute("errorMessage", "Database error occurred while fetching Combos.");
